...
data %>%  drop_na() %>%
          ...
          geom_boxplot() +
          ggtitle("Distribution of Charges by Number of Children") +
          scale_y_continuous(labels=scales::dollar_format()) +
          theme_bw() +
          theme(legend.position="none")  
...
In order to budget properly, an insurance company needs to be able to accurately predict costs.  Let's see how accurately this can be accomplished.  Before any modeling is completed, the correlation coefficient of the continuous predictors (**bmi** and **age**) is checked.  With a low value of 0.11, both predictors can be included in the model without concern of multicollinearity.  
```{r}
