...
* **sex**: female or male
* **bmi**: body mass index
* **num_children**: number of children covered by health insurance / number of dependents
* **smoker**: non-smoker vs. smoker
* **region**: beneficiary's residential area in the US-northeast, southeast, southwest, northwest.

**TARGET:**

* **charges**: Medical costs of insured

### 3) Exploratory Data Analysis 

First, **charges** (target) is explored.  After, each feature is explored with respect to its relationship to **charges**.  
...
data %>%  drop_na() %>%
          ...
          geom_boxplot() +
          ggtitle("Distributions of Charges by Sex") +
...
data %>%  drop_na() %>%
          ...
          geom_boxplot() +
          ggtitle("Distributions of Charges by Smoker") +
          theme_bw() +
          scale_y_continuous(labels=scales::dollar_format()) +
          theme(legend.position="none")  
...
data %>% drop_na() %>%
          ...
          ggtitle("Distributions of Charges by Smoking Status") +
          scale_x_continuous(labels=scales::dollar_format()) +
          theme_bw() 
```
...
data %>% drop_na() %>%
        ggplot(., aes(x=age,y=charges)) + 
               ...
               ggtitle("Scatterplot of of Charges vs. Age") +
               scale_y_continuous(labels=scales::dollar_format()) +
               theme_bw()
...
data %>% drop_na() %>%
        ggplot(., aes(x=age,y=charges, color=smoker)) + 
               geom_jitter(alpha=0.7) +
               ggtitle("Scatterplot of of Charges vs. Age (by Smoking Status)") +
               scale_y_continuous(labels=scales::dollar_format()) +
               theme_bw()
```
...
data %>% drop_na() %>%
        ggplot(., aes(x=bmi,y=charges)) + 
               geom_jitter(alpha=0.7) +
               ggtitle("Scatterplot of of Charges vs. BMI") +
               scale_y_continuous(labels=scales::dollar_format()) +
               theme_bw()
...
data %>% drop_na() %>%
        ggplot(., aes(x=bmi,y=charges, color=smoker)) + 
               geom_jitter(alpha=0.7) +
               ggtitle("Scatterplot of of Charges vs. BMI (by Smoking Status)") +
               scale_y_continuous(labels=scales::dollar_format()) +
               theme_bw()
...
data %>%  drop_na() %>%
          ...
          geom_boxplot() +
          ggtitle("Distributions of Charges by Number of Children") +
          scale_y_continuous(labels=scales::dollar_format()) +
          theme_bw() +
          theme(legend.position="none")  
...
data %>%  drop_na() %>%
          ...
          geom_boxplot() +
          ggtitle("Distributions of Charges by Region") +
          scale_y_continuous(labels=scales::dollar_format()) +
          theme_bw() +
          theme(legend.position="none")  
...
* There is a visually apparent interaction between **smoking** and **bmi**; that is, **charges** increases as **bmi** increases, but at higher rate for a **smoker**

### 4) Predicting Charges

In order to budget properly, an insurance company needs to be able to accurately predict costs.  Let's see if this can be accomplished.  Before any modeling is completed, the correlation coefficient of the continuous predictors (bmi and age) is checked.  This is a low value of 0.11, so both predictors can be included in a model without concern of multicollinearity.  
```{r}
data %>% select(bmi, age) %>% cor()
```

Let's start with some diagnostic linear regression models.  One that uses all of the predictors, and one that uses all of the predictors plus an interaction term between **bmi/smoker**.   
```{r}
...
ggplot(data_train) + geom_density(data=data_train, aes(x=charges, col='blue')) + 
                     ...
                     ggtitle("Distributions of Charges for Training and Validation datasets") +
                     scale_x_continuous(labels=scales::dollar_format())
```
...
Now, the linear regression model with the interaction term is utilized using the training set and 5-fold cross validation is implemented for assessing the effectiveness of the model and its ability of to predict new data prior to using the validation set.  
...
model <- train(charges ~ . + bmi:smoker, data = data_train,
               method = "lm",
...
The model is then utilized to predict charges for both the training and validation datasets.  The **mean absolute error (MAE)** and **Root Mean Squared Error (RMSE)** for predicted charges is calculated for both datasets.
...
For the 400 observations in the validation set, for 323 (81%) observations the actual charges were less than the predicted charges.  With respect to budgeting purposes you would rather pay under a patient's planned budget than over.  However, in the end, looking the percentage of individual patient budgets that were exceeded or not does not give a summary of the big picture.  An insurance company would need to know the cumulative totals from all patients to see if the overall budget was planned appropriately.  Calculations for this scenario in the training and validation sets are below. 
...
The model was able to predict the budget in the validation set with a 0.18% absolute error, and seems that it could be a useful tool to use for budgeting purposes.  More data is needed to validate the model's accuracy and predictive power.    
