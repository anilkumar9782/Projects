...
* There are 318 missing values and from the summary these are all from the _delivery_time_ column
* There is only one value (4.99) for the _delivery_fee_ column -> this column can be deleted
* There is only one value (1) for the _delivery_neighborhood_id_ column -> this column can be deleted
...
The data is re-examined to check that manipulation and cleaning was performed appropriately.  
```{r re-examine data to make sure columns are correct types}
summary(data)
```

From the above summaries, it is apparent that the missing values are for deliveries that were voided (318 voided orders in total).  The 318 missing values for the _Delivery_Over_1hr_ and _Delivery_Time_ columns are due to the fact that these are observations are for voided orders, and therefore don't have an associated delivery time. 

For reference, a summary of post-cleaning data dictionary is below.  
...
* **Delivery_Over_1hr:** Whether order was delivered in over 1 hour vs. 1 hour or less
...
Of the 2320 orders, 318 (13.7%) were voided.  
```{r Order Status counts}
data %>% group_by(Order_Status) %>% summarize(count=n()) %>%
         mutate(freq = round(count / sum(count),3)) 
```

Now, order status is visually explored further with resepect to the other features.    
...
* ONLY on orders where items subsitutions were _NOT_ permitted
* Mostly on weekends
* Mostly on orders with lower GMV's

Let's visually explore the interaction between the *Weekend/Time_of_Day* and the relationship to *Order_Status*.  From the figure below, it's clear that voided orders _ONLY_ occurred after 5pm, and the vast majority were on weekends.
...
Now, delivery time is visually explored further with resepect to the other features.  
...
For the purposes of this project, the majority of possible potential business solutions will not be discussed in detail.  One solution that will be discussed is tackling the delivery time problem by attempting to accurately predict delivery times, so that even if deliveries are going to take longer than one hour at least an approximate time can be given to customers.  This solution is discussed in the section below.    


### 4) Predicting Delivery Times

First, training and validation data sets are created.  The training set consists of 70% of the data and the validation set consists of 30% of the data.  A partition is created so that both the training and validation sets have an equal proportion of observations where the delivery time was over 1 hour.
...
Next, to check that the split was made appropriately.  This is especially important because the data was split on a binary feature and the outcome being looked at is continuous.  Based on the similar overlying density plots and mean delivery times of both of the datasets, an appropriate split was made.  
```{r verify good split}
...
The model below has an Adj. R^2^ of 0.897 compared to the Adj. R^2^ of 0.693 for the simpler model with no interactions terms (the output of this model is not shown).  
...
The above model is used to predict delivery times for both the training and validation datasets.  Next, the **mean absolute errors (MAE)** for predicted delivery times is calculated for both datasets.  
